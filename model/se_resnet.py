...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        original_out = out
        print(original_out.shape, 'original_out')
        out = self.globalAvgPool(out)
        out = out.view(out.size(0), -1)
        print(out.shape,'jjjjjjj')
        out = self.fc1(out)
        out = self.relu(out)
        out = self.fc2(out)
        print(out.shape,'ddddddd')

        out = self.sigmoid(out)
        out = out.view(out.size(0),out.size(1),1,1)
        print(out.shape,'rrrrrrrrrrr')
...
class SENet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        print(x.shape,'11111111')
        x = self.layer2(x)
        print(x.shape,'22222')

        x = self.layer3(x)
        print(x.shape,'33333333')
...
if __name__ == '__main__':
    net = se_resnet_50(num_classes=751)
