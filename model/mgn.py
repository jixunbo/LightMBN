...
class MGN(nn.Module):
    def __init__(self, args):
        ...
        reduction = nn.Sequential(nn.Conv2d(
            2048, args.feats, 1, bias=False), nn.BatchNorm2d(args.feats), nn.ReLU())
    ...
    def forward(self, x):
        ...
        zg_p1 = self.maxpool_zg_p1(p1)  # shape:(batchsize, 2048,1,1)
        zg_p2 = self.maxpool_zg_p2(p2)  # shape:(batchsize, 2048,1,1)
        zg_p3 = self.maxpool_zg_p3(p3)  # shape:(batchsize, 2048,1,1)

        zp2 = self.maxpool_zp2(p2)  # shape:(batchsize, 2048,2,1)
        z0_p2 = zp2[:, :, 0:1, :]
        z1_p2 = zp2[:, :, 1:2, :]

        zp3 = self.maxpool_zp3(p3)  # shape:(batchsize, 2048,3,1)
        z0_p3 = zp3[:, :, 0:1, :]
        z1_p3 = zp3[:, :, 1:2, :]
        z2_p3 = zp3[:, :, 2:3, :]
